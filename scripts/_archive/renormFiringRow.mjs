...
const src = "src/assets/firing_4figures_transparent.png";
const outNorm = "src/assets/firing_4figures_transparent_norm.png";
...
if (width <= 0 || height <= 0) {
  throw new Error("Invalid source size for firing strip.");
}
...
for (let index = 0; index < FRAMES; index++) {
  ...
  const resizedByHeight = await sharp(cropped)
    .resize({
      height: TARGET_H,
      fit: "inside",
  ...
  const resizedByHeightMeta = await sharp(resizedByHeight).metadata();
  const resized =
    (resizedByHeightMeta.width ?? FRAME_W) > FRAME_W
      ? await sharp(resizedByHeight)
          .resize({
            width: FRAME_W,
            fit: "inside",
            background: { r: 0, g: 0, b: 0, alpha: 0 },
  ...
  composites.push({ input: frame, left: index * FRAME_W, top: 0 });
}
...
await sharp({
  ...
  .composite([
    ...
    { input: outNorm, left: 0, top: 512 },
    { input: "src/assets/jumping_5figures_transparent_norm.png", left: 0, top: 768 },
  ])
  .png()
  .toFile(outCombined);
