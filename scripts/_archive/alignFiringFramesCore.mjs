...
function frameStats(frame) {
  ...
  for (let y = 0; y < FRAME_H; y++) {
    for (let x = 0; x < FRAME_W; x++) {
      ...
      const isLikelyWeapon = x > FRAME_W * 0.68;
      const isCoreBand = x >= FRAME_W * 0.18 && x <= FRAME_W * 0.70;

      if (!isMuzzleFlash && !isLikelyWeapon && isCoreBand) {
        coreSumX += x;
        coreCount++;
      }
    }
  ...
  if (maxX < 0) {
    return {
      ...
      width: FRAME_W,
      height: FRAME_H,
      coreCenterX: FRAME_W / 2,
    };
  }

  const width = maxX - minX + 1;
  const height = maxY - minY + 1;
  const coreCenterX = coreCount > 0 ? coreSumX / coreCount : minX + width / 2;

  return { frame, minX, minY, maxX, maxY, width, height, coreCenterX };
}

const stats = firingFrames.map(frameStats);

const sortedCoreCenters = [...stats].map((s) => s.coreCenterX).sort((a, b) => a - b);
const targetCoreCenterX = sortedCoreCenters[Math.floor(sortedCoreCenters.length / 2)];
...
for (const s of stats) {
  ...
  const desiredLeft = Math.round(targetCoreCenterX - (s.coreCenterX - s.minX));
...
console.log(
  JSON.stringify({ file, alignedFrames: firingFrames, targetCoreCenterX, targetBottom, stats }, null, 2),
);
